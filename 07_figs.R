...
mycolors <- c(summer = "#FB6A4A", monsoon = "#0072B2", winter = "#471164FF")

#read in data
dat <- read_csv("data/ebird-elev-residents-WG.csv")
...
#monsoon plot
m_dir_plot <- ggplot(elev_mig) +
  aes(x = reorder(scientific_name, -s_med), xend = scientific_name, color = m_sig, fill = m_sig) +
  geom_point(aes(y = m_med), shape = 
               ifelse(elev_mig$s_m_mig < 0, 25, 
                      ifelse(elev_mig$s_m_mig > 0, 24, 1)), size = 0.5) +
  geom_linerange(aes(ymin = m_med, ymax = s_med), linewidth = 0.2) +
  scale_color_manual(values = c("non_sig" = "black", "sig" = "red")) +
  scale_fill_manual(values = c("non_sig" = "black", "sig" = "red")) +
  theme_pubclean(base_size = 8) +
  labs(y = "Elevation (m)") +
...
ggsave("figs/figure_s1.5_ver00.png", fig_1_plot, height = 200, width = 168, dpi = 800, 
       device = "png", units = "mm")
...
ggsave("figs/figure_s1.6_diet_mass.png", diet_mass_plot, device = "png", 
